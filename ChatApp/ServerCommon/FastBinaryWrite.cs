...
namespace ServerCommon
{
    ...
    {         
        ...
        public static int StringNadLength(byte[] bytes, int offset, string value)
        {
            var lengthPos = offset;
            var length = StringEncoding.UTF8.GetBytes(value, 0, value.Length, bytes, offset+2);
            UInt16(bytes, lengthPos, (UInt16)length);
            return (length+2);
        }

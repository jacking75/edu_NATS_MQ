...
func (mgr* connectionManager) init(maxConnectionCount int, maxUserCount int32) bool {
	...
	for i := 0; i < maxConnectionCount; i++ {
		...
		index := int32(i)
		mgr._connectionList[i].Init(index, CONN_BUFFER_SIZE)
	}
...
func (mgr* connectionManager) addSession(sessionIndex int32, sessionUniqueID uint64) bool {
	if mgr._validSessionIndex(sessionIndex) == false {
...
func (mgr* connectionManager) getSession(sessionIndex int32) *connection {
	if mgr._validSessionIndex(sessionIndex) == false {
		NTELIB_LOG_ERROR("Invalid sessionIndex", zap.Int32("sessionIndex", sessionIndex))
		return nil
	}

	return mgr._connectionList[sessionIndex]
}

func (mgr* connectionManager) getSessionStrictly(sessionIndex int32,
								sessionUniqueID uint64) *connection {
	session := mgr.getSession(sessionIndex);
	if session == nil {
		return nil
	}

	if session.getNetworkUniqueID() != sessionUniqueID {
		return nil
	}
...
func (mgr* connectionManager) _validSessionIndex(index int32) bool {
...
func (mgr* connectionManager) setLogin(sessionIndex int32, sessionUniqueID uint64) bool {
	...
	newUserID := string(session.getUserID())
	if _, ok := mgr._userIDConnectionMap.Load(newUserID); ok {
		return false
	}

	session.setLogin()
	mgr._userIDConnectionMap.Store(newUserID, session)
	atomic.AddInt32(&mgr._currentLoginUserCount, 1)
	return true
}
...
func (mgr* connectionManager) setRoom(sessionIndex int32, sessionUniqueId uint64, roomNumber int32) bool {
	...
	mgr._connectionList[sessionIndex].roomEnter(roomNumber)
	return true
}
